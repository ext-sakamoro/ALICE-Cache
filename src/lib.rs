...
//! - **`no_std`**: `--no-default-features` で `alloc` のある target 統計 counter の
//!   `AtomicU64` は `portable-atomic` 経由なので 64-bit atomic の無い 32-bit MCU
//!   (Cortex-M4 `thumbv7em-none-eabihf`) でも seqlock fallback で動く (CI で両 target を build)
